...
def start(port=21, verbose=0, splash=True):
    ...
    datasocket.bind(('0.0.0.0', int(_DATA_PORT)))
    datasocket.listen(1)
...
def connect_wifi():
    ...
    for index, wifi in enumerate(wifi_list, start=1):
        print('    [{}] {}'.format(index, wifi[0].decode()))
    ...
    password = input('Input password for {}: '.format(wifi_list[selected - 1][0].decode()))

    sta.connect(wifi_list[selected - 1][0].decode(), password)
